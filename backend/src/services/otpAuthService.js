...
class OtpAuthService {
  ...
  async verifyOtp(contact, code) {
    ...
    const user = await this.findUserByContact(identifier, channel, {
      include: [{ model: Farm, as: 'farm', attributes: ['id', 'name', 'status', 'default_purpose', 'country', 'currency', 'timezone'] }]
    });
    ...
    const farm = await Farm.findByPk(invitation.farm_id, { attributes: ['id', 'name', 'status', 'default_purpose', 'country', 'currency', 'timezone'] });
