...
describe('Приглашения в ферму', () => {
  ...
  it('владелец выписывает приглашение — диктовать при этом нечего', async () => {
    ...
    // Зато ответ честно говорит, ушло ли работнику сообщение: почта —
    // опциональная интеграция, и без неё владельцу придётся звать иначе.
    // Само значение зависит от окружения, а вот поле быть обязано —
    // на нём построен весь текст диалога в приложении.
    expect(typeof res.body.data.message_sent).toBe('boolean');
    // Ссылка одна на всех и ничего личного не несёт: с установленным
    // приложением её перехватывает приложение, без него открывается
    // страница с магазинами и веб-версией.
    expect(res.body.data.invite_link).toBe(require('../../src/config/app').inviteUrl);
  });
