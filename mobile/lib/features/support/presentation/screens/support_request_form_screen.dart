...
///
/// Закрывается с `true`, когда обращение ушло: список на предыдущем экране
/// по этому признаку перечитывает себя и сразу показывает новую строку.
class SupportRequestFormScreen extends ConsumerStatefulWidget {
  const SupportRequestFormScreen({super.key});

  @override
  ConsumerState<SupportRequestFormScreen> createState() =>
      _SupportRequestFormScreenState();
}

class _SupportRequestFormScreenState
    extends ConsumerState<SupportRequestFormScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(title: Text(l10n.supportRequestNewTitle)),
        body: ListView(
