...
class _PinSetupScreenState extends ConsumerState<PinSetupScreen> {
  ...
  void _onChanged(String value) {
    ...
    if (_repeat != _first) {
      // Ошибаются не только в повторе: человек мог промахнуться и в первый
      // раз, а тогда повторить «правильно» уже невозможно — он повторяет
      // код, которого сам не знает. Раньше экран оставлял его на повторе
      // навсегда, и единственным выходом была кнопка «не сейчас». Поэтому
      // начинаем набор заново с первого шага.
      setState(() {
