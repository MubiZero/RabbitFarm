...
│   ├── onboarding/                 # ✅ Splash + чек-лист активации на «Сегодня»
...
### Работа без сети — частичная, по уровням

Приложение остаётся преимущественно online-only, но три уровня офлайна уже
реализованы поверх `Dio`-запросов, а не вместо них:

- **Уровень 0 — кэш чтения.** [`ListCache`](../mobile/lib/core/cache/list_cache.dart)
  хранит на диске (`hive`/`hive_flutter`) последнюю первую страницу списков
  стада, клеток и задач, по одному ящику на список. Переживает перезапуск;
  показанный из кэша список помечается баннером
  [`StaleDataBanner`](../mobile/lib/core/widgets/stale_data_banner.dart), как
  только обновление с сервера не удалось.
- **Уровень 1 — детектор связи.** [`isOnlineProvider`](../mobile/lib/core/providers/connectivity.dart)
  (`connectivity_plus`) и [`OfflineBanner`](../mobile/lib/core/widgets/offline_banner.dart)
  показывают отсутствие сети на уровне всего приложения, а не только по
  ошибке очередного запроса.
- **Уровень 2 — очередь отложенных действий.** [`OfflineQueueController`](../mobile/lib/core/offline_queue/offline_queue.dart)
  ставит в очередь на диске (тот же `hive`, отдельный от `ListCache` ящик,
  не чистится при выходе) три операции добавления из
  `FarmCapability.recordDailyWork` — кормление, отметку задачи выполненной,
  заметку — если сети нет в момент сохранения. Отправляются по одной FIFO,
  как только `isOnlineProvider` подтверждает связь; ответ сервера не «нет
  сети» (реальный отказ) снимает элемент с очереди и репортится в Sentry —
  повторами это не лечится. Разрешение конфликта простое, без CRDT: запись
  уходит как есть, слияния с тем, что случилось на сервере за это время,
  нет — риск оправдан только для операций добавления с низкой вероятностью
  конфликта. Изменение существующих записей (правка, удаление) в очередь не
  ставится — только сети без буфера.

Управление сотрудниками, создание/редактирование кроликов и клеток,
транзакции — по-прежнему требуют сети сразу: это редкие операции, либо
требующие актуального состояния сервера, либо и так за ролью, которая
реже работает в поле без связи. Полностью офлайн (без единого успешного
запроса) поголовье/клетки/задачи всё равно не открыть, если кэш уровня 0
ни разу не наполнялся — это осознанное ограничение, а не пропущенная
фича.
...
### Mobile App — 60 экранов

**Статистика (счётчики экранов/маршрутов пересчитаны 2026-09-10, после
переработки онбординга — остальное со сверки 2026-09-09, могло разойтись
по мелочи с независимыми правками между датами):**
...
- 20 репозиториев
- 60 экранов (`presentation/screens/*.dart`)
- 62 маршрута (`GoRoute` в `app_router.dart`)
