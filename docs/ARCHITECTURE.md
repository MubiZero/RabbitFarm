...
│   │   └── platformAdminController.js # Тарифы и фермы — доступно только платформенному админу
│   │
...
│   │   ├── planService.js          # Тарифы и проверка лимитов фермы (кролики, штат)
│   │   └── platformAdminService.js # Список ферм с потреблением, назначение тарифа
│   │
...
│   │   ├── payment.routes.js       # /payments - оплата подписки через Эсхата Мерчант, вебхук банка
│   │   ├── photo.routes.js         # /photos - лента фото по всей ферме
│   │   ├── platform-admin.routes.js # /platform-admin - тарифы и фермы, только для суперадмина
│   │   └── files.routes.js         # /uploads/:folder/:filename - раздача из MinIO,
│   │                                #   монтируется в app.js напрямую, не через этот index.js
│   │
│   ├── validators/                 # ✅ 18 валидаторов Joi + listQuery.js, messages.js (общие хелперы)
...
│   │   ├── paymentValidator.js     # Валидация запроса на оплату
...
Первый кусок подписочной логики поверх уже готовых `Payment`/`Farm`.
Платёж (Эсхата Мерчант) и тариф — независимые понятия: `Plan` описывает
только лимиты, привязка тарифа к оплате (продление, автоотключение по
истечении) не реализована — назначение плана ферме сейчас ручное, через
...
Rabbit/Payment/Invitation и другими таблицами конкретной фермы.
...
**Мобильный UI** — `mobile/lib/features/platform_admin/`: две вкладки
(«Фермы» с потреблением против пределов и назначением тарифа, «Тарифы» —
CRUD). Вход в него — раздел «Платформа» на вкладке «Хозяйство», виден только
при `is_platform_admin` (`isPlatformAdminProvider` в `core/access`, намеренно
...
### Backend API - 128 эндпоинтов

**Статистика:**
- 18 контроллеров
- 23 модели БД
- 20 роутов + index.js (19 под `/api/v1` + `files.routes.js`, раздающий `/uploads` из MinIO напрямую)
- 18 валидаторов Joi (+ listQuery.js, messages.js — общие хелперы)
- JWT аутентификация
- Многоарендность: изоляция по ферме на find/count/aggregate/create/destroy/update (см. раздел «Многоарендность»)
- Тарифы и лимиты: платформенная админка, назначение плана ферме, проверка лимитов кроликов/штата (см. «Тарифы, лимиты и платформенная админка»)
...
- **Модули:** 16/16 (плюс платформенная админка — не фермерский модуль, см. «Тарифы, лимиты и платформенная админка»)
- **API эндпоинты:** 128
- **Модели БД (backend):** 23
- **UI экраны:** 49
- **Backend-тесты:** 1257 (842 юнит + 415 интеграционных, пересчитано 2026-09-08 — интеграционные не перепрогонялись в этой сессии, локальный docker-стек не был поднят)
...
**Project Status**: Активная разработка — базовый функционал, многоарендность, передача хозяйства фермы, push-уведомления, оплата подписки и первый кусок SaaS-логики (тарифы, лимиты, платформенная админка) готовы
