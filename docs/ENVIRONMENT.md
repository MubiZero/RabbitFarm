...
- `SMS_API_TOKEN` — реальный токен Payom, `SMS_TEMPLATE_IDS` — маппинг
  ...
  (`{text-1} - код подтверждения: {code-1}`), и другие SMS приложение не
  шлёт. Отредактировать шаблон нельзя: правка текста даёт новый templateId и
  новую модерацию, поэтому подпись продукта вынесена в `SMS_SENDER_LABEL` —
  её смена модерации не требует.
